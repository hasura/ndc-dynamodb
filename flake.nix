{
  description = "DynamoDB data connector";
      ...
      in
      {
        packages = {
          ...
          docker = pkgs.callPackage ./nix/docker.nix {
            ...
            image-name = "ghcr.io/hasura/ndc-dynamodb";
            tag = "dev";
          };
          ...
          docker-x86_64-linux = pkgs.callPackage ./nix/docker.nix {
            ...
            architecture = "amd64";
            image-name = "ghcr.io/hasura/ndc-dynamodb-x86_64";
          };
          ...
          docker-aarch64-linux = pkgs.callPackage ./nix/docker.nix {
            ...
            architecture = "arm64";
            image-name = "ghcr.io/hasura/ndc-dynamodb-aarch64";
          };
        ...
        apps = {
          ...
          connector = {
            type = "app";
            program = "${self.packages.${localSystem}.default}/bin/ndc-dynamodb";
          };

          cli = {
            type = "app";
            program = "${self.packages.${localSystem}.default}/bin/ndc-dynamodb-cli";
          };
        };

        checks = {
          # Build the crate as part of `nix flake check`
          ndc-dynamodb = self.packages.${localSystem}.default;
        };
